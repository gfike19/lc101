...
function runProgram (scoringAlgorithms) {
    let choice = Number(initialPrompt());
    ...
    choice = Number(input.question("Invlid input, try again: "));
    }
    ...
    while(choice != "stop"){
        ...
        if(choice == 0 || choice < 0 || choice > 2){
            score = scoringAlgorithms[0].scoreFunction(word, newScoreKey);
            scoreName = scoringAlgorithms[0].name;
        }
        else {
